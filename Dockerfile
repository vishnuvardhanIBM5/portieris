# This first stage of the build uses go-toolset to build the portieris binary and creates
...
# Switch to the root user as we need to run yum and rpm to ensure packages are up to date
USER root
RUN yum update -y
...
# Prepare the target rootfs for the scratch container
WORKDIR /
RUN mkdir -p /image && \
...
# Create /tmp for logs and /run for the working directory
